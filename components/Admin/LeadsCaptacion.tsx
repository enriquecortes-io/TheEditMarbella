...
export default function LeadsCaptacion({ password }: Props) {
  ...
  const TH: React.CSSProperties = { padding:"12px 16px", textAlign:"left", fontSize:"11px", fontWeight:700, color:"#4A4540", textTransform:"uppercase", letterSpacing:"0.06em", cursor:"pointer" };
    ...
    <div style={{ padding:"32px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"24px" }}>
        ...
        <div style={{ display:"flex", gap:"8px", alignItems:"center" }}>
          <span style={{ background:"#F2EDE4", padding:"4px 12px", borderRadius:"20px", fontSize:"13px", color:"#4A4540" }}>{leads.length} leads</span>
          <button onClick={()=>setView("kanban")} style={{ padding:"6px 14px", borderRadius:"6px", border:"1px solid #DDD8D0", background:view==="kanban"?"#111827":"white", color:view==="kanban"?"white":"#1A1714", fontSize:"12px", cursor:"pointer" }}>Kanban</button>
          <button onClick={()=>setView("tabla")} style={{ padding:"6px 14px", borderRadius:"6px", border:"1px solid #DDD8D0", background:view==="tabla"?"#111827":"white", color:view==="tabla"?"white":"#1A1714", fontSize:"12px", cursor:"pointer" }}>Tabla</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ background:"white", borderRadius:"8px", border:"1px solid #DDD8D0", overflow:"auto" }}>
              ...
              <tbody>
                {filtered.map((l,i)=>(
                  <tr key={l.id} style={{ borderTop:"1px solid #f3f4f6", background:i%2===0?"white":"#fafafa" }}>
                    ...
                    <td style={{ padding:"12px 16px", fontSize:"12px", color:"#4A4540" }}>{new Date(l.created_at).toLocaleDateString("es-ES")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
